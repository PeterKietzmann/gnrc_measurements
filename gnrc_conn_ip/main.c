...
static void *_server_thread(void *args)
{
    ...
    if (conn_ip_create(&server_conn, &server_addr, sizeof(server_addr), AF_INET6,
                    GNRC_NETTYPE_UNDEF) < 0){
        DEBUG("error initializing socket");
        return NULL;
    }
    server_socket = 0;
    DEBUG("Success: started IP server with family %"PRIu8" and protocol %" PRIu16 "\n", AF_INET6, GNRC_NETTYPE_UNDEF);
    ...
    while (1) {
        ...
        if ((res = conn_ip_recvfrom(&server_conn, server_buffer, sizeof(server_buffer), &src,
                                    &src_len)) < 0) { // oder hier senderport noch rausziehen
